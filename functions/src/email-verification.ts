...
export const sendVerificationEmail = onCall(async (request) => {
  try {
    ...
    logger.info("Sending verification email", {userId, email: emailLower});
...
export const validateVerificationCode = onCall(async (request) => {
  try {
    // Verify user is authenticated
    if (!request.auth) {
      throw new Error("Unauthenticated");
    ...
    logger.info("Validating verification code", {userId});
